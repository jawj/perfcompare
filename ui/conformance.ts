...
export function conformanceUI(el: HTMLElement, parse: (json: string) => any, title: string) {
  let results: ReturnType<typeof testParseAll>;
  let counts: Record<keyof typeof results, number>;
  let allMatched: boolean;
  ...
  m.mount(el, {
    view: () => {
      return m('.conform',
        ...
        ...(!results ? [
          m('div', m.trust(`Ready: ${Object.keys(testsJson).length} tests against <code>JSON.parse</code>`)),
          m('button', {
            onclick: () => {
              results = testParseAll(parse);
              counts = Object.fromEntries(Object.entries(results).map(([k, v]) => [k, v.length])) as Record<keyof typeof results, number>;
              allMatched = results.matchedOutcomes.length === Object.keys(testsJson).length;
            }
          }, 'Run tests')
        ] :
          [
            ...
            counts.validJSONUnexpectedErrors > 0 && m('.valid-throws-error', collapsible(
              m('div', m('b', counts.validJSONUnexpectedErrors), ' errors thrown on valid/ambiguous documents'),
