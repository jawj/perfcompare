...
export function histogram(data: number[][], binCount = 50, size = [400, 100], colors = ['#FF6666', '#33BBFF'], margins = [30, 25, 20, 25], xtitle="ops/ms (higher is better)") {
  ...
  const labelStyle = 'font: 13px sans-serif; margin: 0 4px; letter-spacing: -0.02em;';
...
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 ${size[0]} ${size[1]}"  width="${size[0]}" height="${size[1]}">
  ...
  <!-- x axis -->
  <line x1="0" y1="${baseline}" x2="${size[0]}" y2="${baseline}" stroke="#797D86"/>
  <text dominant-baseline="hanging" text-anchor="middle" x="${size[0] * .5}" y="${baseline + 6}" style="${labelStyle};" fill="#fff">${xtitle}</text>

  <line x1="${margins[3]}" y1="${baseline}" x2="${margins[3]}" y2="${baseline + 3}" stroke="#797D86"/>
  <text dominant-baseline="hanging" text-anchor="middle" x="${margins[3]}" y="${baseline + 6}" style="${labelStyle}" fill="#C9CBCF">${round(min)}</text>

  <line x1="${size[0] - margins[1]}" y1="${baseline}" x2="${size[0] - margins[1]}" y2="${baseline + 3}" stroke="#797D86"/>
  <text dominant-baseline="hanging" text-anchor="middle" x="${size[0] - margins[1]}" y="${baseline + 6}" style="${labelStyle}" fill="#C9CBCF">${round(max)}</text>
